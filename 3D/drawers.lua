...
local draw_scn_direct
draw_scn_direct=function(scn)
...
Drawer_hooks("scn",draw_scn_direct)
...
local draw_meta_stick=function(r,n)
	r=r or 1
	n=n or 10
	local sin,cos,rad=math.sin,math.cos,math.rad
	...
	for i,ang in ipairs(angs) do
		...
		API.set_vertex(x,y,1,u,1,dx,dy,0)
		API.set_vertex(x,y,0,u,0,dx,dy,0)
	end
	API.end_draw()
end
Drawer_hooks("meta-stick",draw_meta_stick)


local stick_coord
draw_stick=function(from,to,r,n)
	local v=sub(to,from)
	...
	Y=cross(Z,X)
	X=cross(Y,Z)
	stick_coord=build_coord_XYZT(normalize(X),normalize(Y),v,from,stick_coord)
...
Drawer_hooks("stick",draw_stick)

require "plugins/nurbs"
